# DESIGN.md

Verified against code on 2026-08-07.

Use this file as the design-doc entry point. Start small, then drill down.

## Read Order

1. [Core Beliefs](./design-docs/core-beliefs.md)
2. [Design Catalog](./design-docs/index.md)
3. [Architecture](../ARCHITECTURE.md)
4. relevant deep doc for the area you are touching

## Design Scope

Design docs explain why the system is shaped this way:

- agent-first operating principles
- runtime model and package boundaries
- source adapter rules and drift handling
- areas intentionally deferred or constrained

## Current Deep Docs

- [Core Beliefs](./design-docs/core-beliefs.md)
- [Runtime Model](./design-docs/runtime-model.md)
- [Source Adapters](./design-docs/source-adapters.md)

## Related Guides

- [Plans](./PLANS.md)
- [Product Sense](./PRODUCT_SENSE.md)
- [Quality Score](./QUALITY_SCORE.md)
- [Reliability](./RELIABILITY.md)
- [Security](./SECURITY.md)
